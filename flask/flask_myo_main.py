...
class EMGListener(myo.DeviceListener):
    ...
    def on_emg(self, event):
        with self.lock:
            if self.is_saving:
                self.emg_data = [round(event.timestamp/1000-self.start_time_ms, 4)] + event.emg
                self.add_emg()
    ...
    def start_saving(self, t):
        self.start_time_ms = t
        self.is_saving = True
...
class EMGPlot(object):
    def __init__(self, emg_listener, pygame):
...
def run_pygame():
    ...
    global myoHub, myoListener, emgPlotter
    try:
    ...
    emgPlotter = EMGPlot(myoListener, pygame)

    with myoHub.run_in_background(myoListener.on_event):
        emgPlotter.init_pygame()

        finished = False
        while not finished:
            ## update
            emgPlotter.update_plot()
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        myoListener.start_saving(start_time_ms)
                        is_saving = True
                    elif event.key == pygame.K_RIGHT:
                        myoListener.save_emg()
                        print('Data Saved')
            
            if is_saving:
                delta_t = time.time()-start_time_s
                if delta_t >= time_recording:
                    is_saving = False
                    myoListener.stop_saving()
                    myoListener.save_emg()
                    print('Data Saved: ' + str(delta_t))
